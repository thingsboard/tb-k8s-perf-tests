...
# nodes=($(cat test-out.txt | grep 'tb-node-' | awk '{print $1}'))

len=${#nodes[@]}

echo Found $len tb-nodes...
if [ "$len" -eq "0" ]; then
   echo "No tb-nodes pods found! Exiting...";
   exit 1;
fi
...
do
    node=${nodes[$i]}
    echo $i: $node
    kubectl logs -f $node --since=1m &> tb-node-logs/$node.log &
done
